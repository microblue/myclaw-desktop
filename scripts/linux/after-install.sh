...
# Post-installation script for MyClaw on Linux
...
# Create symbolic link for MyClaw app binary
if [ -x /opt/MyClaw/clawx ]; then
    ln -sf /opt/MyClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/MyClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/MyClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/MyClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/MyClaw/resources/apparmor-profile'
...
echo "MyClaw has been installed successfully."
