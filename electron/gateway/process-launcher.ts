...
(function () {
  ...
  if (globalThis.__myclawFetchPatched) return;
  globalThis.__myclawFetchPatched = true;

  globalThis.fetch = function myclawFetch(input, init) {
    var url =
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__myclawPatched) {
        cp.__myclawPatched = true;
