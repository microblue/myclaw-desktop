...
interface SyncProxyOptions {
   ...
   * currently disabled in MyClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync MyClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because MyClaw proxy is disabled and preserve mode is enabled');
      return;
    }
