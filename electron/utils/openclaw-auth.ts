 ...
 * Used by MyClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the MyClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged MyClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  MyClaw sets
 ...
 * OpenClaw/MyClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When MyClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for MyClaw to properly integrate with its updated tool system.
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // id="openclaw-qqbot" in its manifest, but older MyClaw versions
    ...
    // but MyClaw historically stored them only under `channels.<type>.accounts.default`.
