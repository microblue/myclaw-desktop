...
/**
 * Merge a MyClaw context section into an existing file's content.
...
/**
 * Detect and remove bootstrap .md files that contain only MyClaw markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairMyClawOnlyBootstrapFiles(): Promise<void> {
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed MyClaw-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove MyClaw-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge MyClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeMyClawContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('MyClaw context directory not found, skipping context merge');
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      ...
      const merged = mergeMyClawSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged MyClaw context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure MyClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureMyClawContext(): Promise<void> {
  let skipped = await mergeMyClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeMyClawContextOnce();
    if (skipped === 0) {
      logger.info(`MyClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`MyClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`MyClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
